...
content_ids_df = pd.read_csv('stay_site_id.csv')
...
for idx, content_id in enumerate(content_ids, start=1):
    url = f"{base_url}{content_id}&menuId=DOM_000001707000000000#p1"
...
# CSV로 저장
df.to_csv('stay_site_data.csv', index=False, encoding='utf-8-sig')

# 드라이버 종료
driver.quit()

print("데이터 수집 완료! 'stay_site_data.csv' 파일로 저장되었습니다.")
